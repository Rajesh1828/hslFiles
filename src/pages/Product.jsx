...
const Product = () => {
  ...
  const { currency, collection } = useContext(FileContext);
            ...
            <div className='flex flex-col gap-4 my-8'>
              ...
              <div className='flex gap-4'>
                {product.size.map((item, index) => (
                  <button onClick={() => setSize(item)} className={`border-1 cursor-pointer rounded-md border-gray-600 px-1 py-1 ${size === item ? 'bg-gray-600 text-white' : ''}`} key={index}>{item}</button>
                ))}

              </div>
            </div>
            <p className='text-md text-gray-800'>Model: {product.model}  </p>
            <p className='text-md text-gray-800'>Brand: {product.brand}  </p>
          </div>
